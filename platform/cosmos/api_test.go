...
var cosmosValidator = CosmosValidator{Commission: Commission{Rate: "0.4"}}
...
func TestNormalizeValidator(t *testing.T) {
	...
	expected := blockatlas.Validator{
		...
		Tokens: blockatlas.Tokens{Bounded: 9538882295763},
	}

	result, _ := normalizeValidator(v, stakingPool, inflation, coin)
...
func TestCalculateAnnualReward(t *testing.T) {
	...
	result := CalculateAnnualReward(StakingPool{"1222", "200"}, inflation, CosmosValidator{Commission: Commission{Rate: "0.4"}})
